...
#define KEY_CONDITION  "GScriptCondition"
...
bool GScriptCondition::loadAndExecuteScript()
{
	...
	{
		adjustScriptFileName(m_scriptFile);
		chunkName = m_scriptFile;

		auto fileUtils = FileUtils::getInstance();
		std::string fullPathOfScript = fileUtils->fullPathForFilename(m_scriptFile);
