...
float Vec3::angle(const Vec3& v1, const Vec3& v2)
{
    ...
    return std::atan2(std::sqrt(dx * dx + dy * dy + dz * dz) + MH_FLOAT_SMALL, dot(v1, v2));
}
...
{
    MH_ASSERT(!(min.x > max.x || min.y > max.y || min.z > max.z));
...
void Vec3::cross(const Vec3& v3_1, const Vec3& v3_2, Vec3* dst)
{
    MH_ASSERT(dst);
	...
	const float* v1 = &v3_1.x;
	const float* v2 = &v3_2.x;

	dst->x = (v1[1] * v2[2]) - (v1[2] * v2[1]);
	dst->y = (v1[2] * v2[0]) - (v1[0] * v2[2]);
	dst->z = (v1[0] * v2[1]) - (v1[1] * v2[0]);
}
