                                   ...
                                   Mat4* dst)
{
    ...
    bool isSufficientDelta = delta.lengthSquared() > MH_EPSILON;
...
bool Mat4::decompose(Vec3* scale, Quaternion* rotation, Vec3* translation) const
{
    ...
    if (scaleX < MH_TOLERANCE || scaleY < MH_TOLERANCE || std::abs(scaleZ) < MH_TOLERANCE)
        return false;
