...
Vec2 CRect::getVertexMin()
{
	apply();
	Vec2 min = vertex_vec2_value[0];

	for (auto i = 1; i < 4; ++i)
	{
		if (vertex_vec2_value[i].x < min.x)
			min.x = vertex_vec2_value[i].x;

		if (vertex_vec2_value[i].y < min.y)
			min.y = vertex_vec2_value[i].y;
	}
	return min;
}

Vec2 CRect::getVertexMax()
{
	apply();
	Vec2 max = vertex_vec2_value[0];

	for (auto i = 1; i < 4; ++i)
	{
		if (vertex_vec2_value[i].x > max.x)
			max.x = vertex_vec2_value[i].x;

		if (vertex_vec2_value[i].y > max.y)
			max.y = vertex_vec2_value[i].y;
	}
	return max;
}

bool CRect::isAABBBox()
{
	if (getRotation() != 0.0f)
		return false;
